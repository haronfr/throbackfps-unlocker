...
std::vector<HANDLE> GetThrobackProcesses(bool include_client = true, bool include_studio = true)
{
...
HANDLE GetThrobackProcesses()
{
	auto processes = GetThrobackProcesseses();
...
DWORD WINAPI WatchThread(LPVOID)
{
	...
	while (1)
	{
		auto processes = GetThrobackProcesseses(Settings::UnlockClient, Settings::UnlockStudio);
